...
def cleanup():
    def delete_leftovers():
        ...
        for m in app_modes:
            fp = os.path.join(
                TEST_STUDY_DIR, "extract_configs", f"extract_config_{m}.py"
            )
...
def extract_config(app_mode):
    ...
    src = os.path.join(TEST_STUDY_DIR, f"extract_config_{app_mode}.py")
    dest = os.path.join(
        TEST_STUDY_DIR, "extract_configs", f"extract_config_{app_mode}.py"
    )
...
def test_non_prod_app_modes(
    info_caplog, cleanup, source_data_file, cached_schema, app_mode, host
):
    """
...
def test_prod_app_mode(info_caplog, cleanup):
    ...
    # Run and test ingest
    app_mode = "production"
    app_settings = settings.load(os.path.join(SETTINGS_DIR, app_mode + ".py"))
