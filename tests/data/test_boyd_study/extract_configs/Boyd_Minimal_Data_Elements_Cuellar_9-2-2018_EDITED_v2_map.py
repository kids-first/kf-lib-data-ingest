...
operations = [
    value_map(
        ...
        m={r".*": lambda x: _sample_mapping.get(x, x)},
    ...
    keep_map(in_col="Family Group ID", out_col=CONCEPT.FAMILY.ID),
    value_map(
    ...
    value_map(
        m={r"osteo": constants.SPECIMEN.ANATOMY_SITE.SKULL},
        in_col="Tissue from notation",
    ...
    value_map(
        m={
            ...
            r"Maternal Grandmother": constants.RELATIONSHIP.MATERNAL_GRANDMOTHER,
    ...
    keep_map(in_col="Phenotypes Text", out_col=CONCEPT.PHENOTYPE.NAME),
    keep_map(in_col="Phenotypes HPO", out_col=CONCEPT.PHENOTYPE.HPO_ID),
    keep_map(in_col="Diagnoses Text", out_col=CONCEPT.DIAGNOSIS.NAME),
    value_map(
        m={r"(.+)": lambda x: "NCIT:" + x},
        in_col="Diagnosis NCIT",
        out_col=CONCEPT.DIAGNOSIS.NCIT_ID,
    ),
    keep_map(in_col="Vital Status", out_col=CONCEPT.OUTCOME.VITAL_STATUS),
]
