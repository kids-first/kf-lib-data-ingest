...
operations = [
    ...
    keep_map(
        in_col="proteomics_experiment",
        out_col=CONCEPT.SEQUENCING.PROTEOMICS_EXPERIMENT,
    ),
    keep_map(
        "mass_spec_rawfile_conversion",
        out_col=CONCEPT.SEQUENCING.MASS_SPEC_RAWFILE_CONVERSION,
    ),
    constant_map(
    ...
    keep_map(
        "enrichment_approach", out_col=CONCEPT.SEQUENCING.ENRICHMENT_APPROACH
    ),
    keep_map(
        "quantification_technique",
        out_col=CONCEPT.SEQUENCING.QUANTIFICATION_TECHNIQUE,
    ),
    keep_map(
        "quantification_labeling_method",
        out_col=CONCEPT.SEQUENCING.QUANTIFICATION_LABELING_METHOD,
    ),
    keep_map(
        "quantification_label_id",
        out_col=CONCEPT.SEQUENCING.QUANTIFICATION_LABEL_ID,
    ),
    keep_map(
        "chromatography_approach",
        out_col=CONCEPT.SEQUENCING.CHROMATOGRAPHY_APPROACH,
    ),
    keep_map(
        "fractionation_approach",
        out_col=CONCEPT.SEQUENCING.FRACTIONATION_APPROACH,
    ),
