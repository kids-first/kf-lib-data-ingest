...
def test_merge_wo_duplicates(info_caplog, dfs):
    ...
    merged = pandas_utils.merge_wo_duplicates(
        df1, df2, left_name="df1", right_name="df2", on="A", how="outer"
    )
    ...
    with pytest.raises(Exception) as e:
        pandas_utils.merge_wo_duplicates(
            df1, df2, left_on="__lkasjdf__", right_on="A"
        )
    assert "not found in Left" in str(e.value)
    ...
    merged = pandas_utils.merge_wo_duplicates(
        df1, df1, left_on="A", right_on="A", how="outer"
    )
...
def test_merge_wo_duplicates_collisions(info_caplog):
    ...
    assert out.equals(
        pandas.DataFrame({"a": [1, 2, 3], "b": ["a", "b", "c"]}, dtype=object)
    )
...
def test_outer_merge(info_caplog, dfs):
    ...
    ret_val = pandas_utils.outer_merge(
        df1, df2, left_name="df1", right_name="df2", on="A"
    )
...
def test_split_df_rows():
    ...
    df_delims = pandas.DataFrame({"a": ["1,2"], "b": ["3,4,5"], "c": ["6,7"]})
    ...
    for out in [out_on_splits, out_on_delims]:
        ...
        assert (
            ...
            .equals(out.reset_index(drop=True))
        )
        # now compare indices
