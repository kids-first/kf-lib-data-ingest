...
class OAuth2Mocker(object):
    ...
    def get_service_token(self,
        ...
        with requests_mock.Mocker() as m:
            self.create_service_token_mock(m,
                                           provider_domain,
                                           audience,
                                           client_id,
                                           client_secret,
    ...
    def get_file(self, url, dest_obj,
                 provider_domain=TEST_AUTH0_DOMAIN,
                 audience=TEST_AUTH0_AUD,
                 client_id=TEST_CLIENT_ID,
                 client_secret=TEST_CLIENT_SECRET,
    ...
    def create_get_file_mock(self, m, url, dest_obj,
                             provider_domain=TEST_AUTH0_DOMAIN,
                             audience=TEST_AUTH0_AUD,
                             client_id=TEST_CLIENT_ID,
                             client_secret=TEST_CLIENT_SECRET,
    ...
    def create_service_token_mock(self, m,
                                  provider_domain=TEST_AUTH0_DOMAIN,
                                  audience=TEST_AUTH0_AUD,
                                  client_id=TEST_CLIENT_ID,
                                  client_secret=TEST_CLIENT_SECRET,
        ...
        if not mock_kwargs.get('json'):
            ...
            if expected_status == 200:
                mock_kwargs['json'] = {'access_token': 'the token',
                                       'expires_in': 5,
                                       'scope': 'download:file',
                                       'token_type': 'Bearer'
                                       }
