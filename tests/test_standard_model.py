...
def data_df_dict(auto_transform_stage):
    ...
    df_dict = {
        f"s3://bucket/key/{k}.csv": (
            f"file:///study/configs/{k}.py",
            pd.DataFrame(v),
        )
...
def model(data_df_dict):
    ...
    export_to_gml(
        model.concept_graph, os.path.join(TEST_DATA_DIR, "test_graph.gml")
    )
...
def random_data_df_dict(auto_transform_stage):
    ...
    data_dict = {
        CONCEPT.FAMILY.UNIQUE_KEY: [
            random.choice(family_id_pool) for i in range(n_participants)
        ],
        CONCEPT.PARTICIPANT.UNIQUE_KEY: [
            f"P{i}" for i in range(n_participants)
        ],
        CONCEPT.BIOSPECIMEN.UNIQUE_KEY: [
            f"BG{i}-B{i}" for i in range(n_participants)
        ],
        CONCEPT.PARTICIPANT.RACE: [
            random.choice(races) for i in range(n_participants)
        ],
        CONCEPT.BIOSPECIMEN.COMPOSITION: [
            random.choice(compositions) for i in range(n_participants)
        ],
    }
    clinical = pd.DataFrame(data_dict)
    df_dict = {
        f"s3://bucket/key/clinical.csv": (
            f"file:///study/configs/clinical.py",
            clinical,
        )
    }
...
def test_transform_all(target_api_config, random_model):
    ...
    data = random_model.transform(
        target_api_config, target_concepts_to_transform=None
    )
...
def test_is_neighbor_valid(
    target_api_config, model, node_concept, neighbor_key, expected_output
):
    """
...
def test_find_attribute_value(
    target_api_config, model, id_node_key, concept_attr, expected_output
):
    ...
    assert (
        g.find_attribute_value(start_node, concept_attr, rg) == expected_output
    )
...
def test_gml_import_export(tmpdir_factory, model):
    ...
    for node_key in model.concept_graph.graph.nodes:
        ...
        assert node
        assert ConceptNode.to_dict(node["object"]) == ConceptNode.to_dict(
            orig_node["object"]
        )
